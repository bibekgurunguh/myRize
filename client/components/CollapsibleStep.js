...
export default function CollapsibleStep(props) {
  ...
  if (
    USER.my_resolutions
      .filter((el) => el.resolution === props.chosenRes)[0]
      .completedSteps.includes(props.stepId)
  )
    initiallyAdded = true;
    ...
    <View style={styles.container}>
      ...
      <View
        style={[
          ...
          {
            opacity:
              props.stepId <= props.nextStep || initialNextStep ? 1 : 0.6,
          },
        ]}
      >
        <View
          style={[
            ...
            {
              backgroundColor:
                initiallyAdded || stepAdded ? "lime" : Theme.primary,
            },
          ]}
        >
      ...
      <View style={[styles.descriptionView, { height: anim }]}>
        ...
        <View style={styles.descriptionNav}>
          <TouchableOpacity
            onPress={() => descNumber > 1 && setDescNumber(descNumber - 1)}
          >
            <Text
              style={[
                styles.description,
                { padding: 0, width: 50, textAlign: "center" },
              ]}
            >
              &#10094;
            </Text>
          ...
          <Text style={styles.descriptionNumber}>
            {descNumber}/{props.content.length}
          </Text>
          <TouchableOpacity
            onPress={() =>
              descNumber < props.content.length && setDescNumber(descNumber + 1)
            }
          >
            <Text
              style={[
        ...
        {notes
          .filter(
            (note) =>
              note.stepId === props.stepId &&
              note.resolution === props.chosenRes
          )
          .map((note) => {
            return (
              <View
                key={note._id}
                style={[styles.noteView, { elevation: elev }]}
              >
        ...
        {step === "expanded" && (
          <View
            style={[
              noteNav ? styles.noteView : styles.noteSymbol,
              { elevation: elev, padding: 10, paddingTop: 0 },
            ]}
          >
            <TouchableOpacity
              activeOpacity={0.5}
